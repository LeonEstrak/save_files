...
# Function to get the most recent backup date from ludusavi zip filenames
# Zip files are named: backup-YYYYMMDDTHHMMSSZ.zip
def get_latest_backup_date(game_dir_path):
    try:
        zips = [f for f in os.listdir(game_dir_path) if f.startswith('backup-') and f.endswith('.zip')]
        if not zips:
            return None
        latest = sorted(zips)[-1]
        ts_str = latest[len('backup-'):-len('.zip')]
        return datetime.strptime(ts_str, '%Y%m%dT%H%M%SZ')
    except Exception as e:
        print(f"Exception getting backup date for {game_dir_path}: {e}")
        return None
...
def generate_readme(base_path):
    game_backups_path = os.path.join(base_path, 'game_backups')

    # Create Cache
    games = read_existing_readme(os.path.join(base_path,"README.md"))

    for folder in os.listdir(game_backups_path):
        folder_path = os.path.join(game_backups_path, folder)
        if os.path.isdir(folder_path):
            last_played_date = get_latest_backup_date(folder_path)
